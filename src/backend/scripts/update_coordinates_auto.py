...
def get_games_to_reprocess(connection):
    cursor = connection.cursor()
    try:
        cursor.execute("""
            SELECT DISTINCT pd.platform_game_id, t.tournament_type
            FROM player_died pd
            JOIN game_mapping gm ON pd.platform_game_id = gm.platform_game_id
            JOIN tournaments t ON gm.tournament_id = t.tournament_id AND gm.tournament_type = t.tournament_type
            WHERE pd.killer_x IS NULL
            AND t.tournament_type IN ('vct-challengers');
        """)
        return cursor.fetchall()
    except Exception as e:
        logger.error(f"Error getting games to reprocess: {e}")
        return []
    finally:
        cursor.close()
...
def main():
    ...
    games_to_reprocess = get_games_to_reprocess(connection)
    connection.close()

    logger.info(f"Found {len(games_to_reprocess)} games to reprocess")

    for platform_game_id, tournament_type in games_to_reprocess:
        logger.info(f"Starting update for game ID: {platform_game_id} in tournament: {tournament_type}")
        update_specific_game(tournament_type, platform_game_id)

    logger.info("Completed game events coordinate update for all identified games")
